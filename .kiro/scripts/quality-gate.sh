...
# Used by: .kiro/hooks/quality-gate.kiro.hook (userTriggered)
...
if command -v npx &>/dev/null && [ -f "tsconfig.json" ]; then
  ...
  if command -v pyright &>/dev/null; then run_check "Type check" pyright
  elif command -v mypy &>/dev/null; then run_check "Type check" mypy .
  else echo "$SKIP Type check (pyright/mypy not installed)"; SKIPPED=$((SKIPPED + 1)); fi
else
  echo "$SKIP Type check (no config found)"
  SKIPPED=$((SKIPPED + 1))
fi
